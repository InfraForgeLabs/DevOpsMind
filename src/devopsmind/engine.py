...
# ---------------------------------------------------------
def play(ch_id: str, context: Dict[str, Any] | None = None, return_data: bool = False):
    ...
    # Build submission payload (per workflow expectations)
    st = load_state()
    player = st.get("player", {})
    email = str(player.get("email", "")).strip()

    submission = {
        "gamer": player.get("gamer") or player.get("name") or "player",
        "username": email or player.get("name") or "player",
        "xp": int(ch.xp),                       # xp for THIS challenge (merge logic adds when new)
        "rank": player.get("rank", "Beginner"),
        "completed": [ch.id],
        "email": email,                         # used to derive email_hash in builder
        "timestamp": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
    }

    return (True, submission) if return_data else True
